...
The SDK method surface is intentionally small: declare a name, typed params, the contract
fields your method consumes, the output fields/panel it produces, and a `compute()` function.
...
`compute()`, coerces params into `params_type`, and rejects results that do not contain every
declared `output.fields` path. If `output.panel` is present, its `field` must point at one of
the declared output fields so the UI can expose the result without guessing.
